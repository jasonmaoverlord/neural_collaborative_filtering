...
class NeuMF(nn.Module):
    def __init__(self, num_users, num_items, mf_dim, layers, reg_layers, reg_mf, user_texts=None, item_texts=None):
        ...
        # Initialize text embeddings if user_texts and item_texts are provided
        if user_texts and item_texts:
            ...
            with torch.no_grad():
                user_embeddings = self._get_text_embeddings(user_texts)
                item_embeddings = self._get_text_embeddings(item_texts)

            # GMF Embedding Layers initialized with text embeddings
            self.mf_user_embedding = nn.Embedding.from_pretrained(user_embeddings, freeze=False)
            self.mf_item_embedding = nn.Embedding.from_pretrained(item_embeddings, freeze=False)

            # MLP Embedding Layers initialized with text embeddings
            self.mlp_user_embedding = nn.Embedding.from_pretrained(user_embeddings, freeze=False)
            self.mlp_item_embedding = nn.Embedding.from_pretrained(item_embeddings, freeze=False)
        else:
